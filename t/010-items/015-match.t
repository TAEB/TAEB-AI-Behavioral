...
ok($balsa->_match("identity", undef, undef), "undef should match undef!");
ok(!$balsa->_match("identity", undef, 'foo'), "seeking undef, found something");
ok(!$balsa->_match("identity", 'foo', undef), "undef shouldn't match anything");
...
ok($balsa->identity eq 'wand of teleportation');
